...
/// </summary>
public class KsqlDbTweetProcessor
{
    private readonly IKSqlDbContextFactory factory;
    private readonly KsqlDbConfiguration configuration;
    
    public KsqlDbTweetProcessor(IOptions<KsqlDbConfiguration> options, IKSqlDbContextFactory factory)
    {
    ...
    private IDisposable LatestByOffset(IApplicationKSqlDbContext context)
    {
        var query = context.Tweets
          .GroupBy(c => c.Id)
          .Select(g => new { Id = g.Key, EarliestByOffset = g.EarliestByOffset(c => c.Message, 2) })
          .ToQueryString();
